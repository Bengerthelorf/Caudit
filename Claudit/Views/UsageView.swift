            ...
            VStack(alignment: .leading, spacing: 14) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Today")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text(ClauditFormatter.costDetail(appState.todayUsage.totalCost))
                        .font(.system(size: 28, weight: .semibold, design: .rounded))
                        .monospacedDigit()
                }
                .accessibilityElement(children: .combine)
                .accessibilityLabel("Today's cost: \(ClauditFormatter.costDetail(appState.todayUsage.totalCost))")
                        ...
                        ForEach(appState.modelBreakdown) { entry in
                            HStack {
                                Text(entry.model)
                                    .font(.system(size: 11))
                                Spacer()
                                Text(ClauditFormatter.tokens(entry.totalTokens))
                                    .font(.system(size: 11))
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text(label)
                ...
                .foregroundStyle(.secondary)
            Spacer()
            Text(ClauditFormatter.tokens(tokens))
                .font(.system(size: 11))
                .monospacedDigit()
                .foregroundStyle(.tertiary)
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            Text(title)
                .font(.system(size: 10))
                .foregroundStyle(.tertiary)
            Text(ClauditFormatter.costDetail(cost))
                .font(.system(size: 14, weight: .medium, design: .rounded))
                .monospacedDigit()
            Text(ClauditFormatter.tokensWithUnit(tokens))
                .font(.system(size: 10))
                .foregroundStyle(.tertiary)
        }
    }
